...
CREATE TABLE tbl_book (
    book_BookID SERIAL PRIMARY KEY,
    book_Title VARCHAR(100) NOT NULL,
    book_PublisherName VARCHAR(100) NOT NULL
);

CREATE TABLE tbl_library_branch (
    library_branch_BranchID SERIAL PRIMARY KEY,
    library_branch_BranchName VARCHAR(100) NOT NULL,
    library_branch_BranchAddress VARCHAR(200) NOT NULL
);

CREATE TABLE tbl_borrower (
    borrower_CardNo SERIAL PRIMARY KEY,
...
CREATE TABLE tbl_book_loans (
    book_loans_LoansID SERIAL PRIMARY KEY,
...
CREATE TABLE tbl_book_copies (
    book_copies_CopiesID SERIAL PRIMARY KEY,
...
CREATE TABLE tbl_book_authors (
    book_authors_AuthorID SERIAL PRIMARY KEY,
...
RETURNS TABLE (
  "Branch ID" integer,
  "Branch Name" varchar(70),
  "Number of Copies" integer,
  "Book Title" varchar(70)
)
AS $$
BEGIN
  RETURN QUERY
...
CREATE OR REPLACE FUNCTION bookCopiesAtAllBranches(bookTitle varchar(70) = 'The Lost Tribe')
RETURNS TABLE (
  "Branch ID" integer,
  "Branch Name" varchar(70),
  "Number of Copies" integer,
...
RETURNS TABLE (borrower_BorrowerName varchar(70))
AS $$
BEGIN
  RETURN QUERY
  SELECT borrower.borrower_BorrowerName
  FROM tbl_borrower AS borrower
  LEFT JOIN tbl_book_loans AS book_loans
...
RETURNS TABLE (
  ...
  "Borrower Name" varchar(70),
  "Borrower Address" varchar(100),
  "Date Out" date,
  "Due Date" date
)
AS $$
...
SELECT * FROM LoanersInfo(current_date, 'Sharpstown');
...
CREATE OR REPLACE FUNCTION public."BooksLoanedOut"(
    BooksCheckedOut INT = 5
)
RETURNS TABLE (
    "Borrower Name" VARCHAR(255),
    "Borrower Address" VARCHAR(255),
    "Books Checked Out" INT
)
AS $$
BEGIN
    RETURN QUERY
...
SELECT * FROM public."BooksLoanedOut"();


/* #7- For each book authored by "Stephen King", retrieve the title and the number of copies owned by the library branch whose name is "Central".*/

CREATE OR REPLACE FUNCTION public."BookbyAuthorandBranch"(
...
SELECT * FROM public."BookbyAuthorandBranch"('Central', 'Stephen King');
